  ...
  const ProfileContent = () => (
    <>
        ...
        <div className="flex flex-col gap-0 items-start justify-start w-full max-w-full overflow-hidden">
          ...
          <span className="w-full h-fit items-center justify-start gap-x-1 flex">
            <Avatar className="size-4 rounded-none">
              <AvatarImage
                src={branch.business.business_logo_url}
                alt={branch.business.business_name}
              />
              <AvatarFallback className="rounded-none">
                {branch.branch_name.slice(0, 2)}
              </AvatarFallback>
            </Avatar>
            <p className="truncate font-normal w-full">{branch.branch_name}</p>
          </span>
        </div>
      </div>
      <div className="p-1 border-b">
        ...
        >
          <Link
            href={`/settings/${user.id}/${branch.id}/wedding`}
            className="flex items-center justify-between w-full"
            onClick={() => isMobile && setSheetOpen(false)}
          >
